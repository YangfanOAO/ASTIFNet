...
class Exp_Classification(Exp_Basic):
    ...
    def _build_model(self):
        ...
        for batch_x, label, padding_mask in test_loader:
            batch_x = batch_x[0].unsqueeze(0).to(self.device)
            padding_mask = padding_mask[0].unsqueeze(0).to(self.device)
            self.flops, self.params = profile(model_copy, inputs=(batch_x, padding_mask, None, None), verbose=False)
            del model_copy
            break
    ...
    def train(self, setting):
        ...
        path = "./checkpoints/" + self.args.task_name + "/" + self.args.model_id + "/" + self.args.model + "/" + setting + "/"
        ...
        for epoch in range(self.args.train_epochs):
            ...
            early_stopping(-val_metrics_dict["F1"], self.swa_model if self.swa else self.model, path, )
    ...
    def test(self, setting, test=0):
        ...
        # run_time=run_time/len(vali_loader)
        ...
        file_name = "result_acc.txt"
        f = open(os.path.join(folder_path, file_name), "a")
        # f.write(setting + "  \n")
        # f.write("\n")
        f.write(f"{test_metrics_dict['Accuracy']:.5f}")
        f.write("\n")
        f.close()

        return test_metrics_dict, run_time
