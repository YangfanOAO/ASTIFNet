...
import random
import numpy as np
from utils.tools import result_save, str2bool
from thop import clever_format

if __name__ == "__main__":

    parser = argparse.ArgumentParser(description="Medical Time-Series Classification")

    # task
    parser.add_argument("--task_name", type=str, default="classification")
    parser.add_argument("--is_training", type=int, default=0, help="status train 1 test 0")
    parser.add_argument("--model", type=str, default="ASTIFNet", help="model name")

    # data loader
    parser.add_argument("--model_id", type=str, default="TDBRAIN-Indep", help="model id")
    parser.add_argument("--data", type=str, default="TDBRAIN")
    parser.add_argument("--root_path", type=str, default="./dataset/TDBRAIN/")
    parser.add_argument("--freq", type=str, default="h")

    # ASTIFNet
    parser.add_argument('--emb_size', nargs='+', type=int, default=[7, 8, 9])
    parser.add_argument('--kernel_t', nargs='+', type=int, default=[5, 6, 5])
    parser.add_argument('--kernel_s', nargs='+', type=int, default=[4, 3, 4])
    ...
    parser.add_argument("--embed", type=str, default="timeF", help=" options:[timeF, fixed, learned]")
    parser.add_argument("--activation", type=str, default="gelu", help="activation")
    parser.add_argument("--output_attention", action="store_true", help="whether to output attention in encoder")
    parser.add_argument("--no_inter_attn", action="store_true", default=False)
    ...
    parser.add_argument("--patch_len_list", type=str, default="2,4,8", help="a list of patch len used in Medformer")
    parser.add_argument("--single_channel", action="store_true", default=False, help="single channel patching ")
    parser.add_argument("--augmentations", type=str, default="flip,shuffle,frequency,jitter,mask,drop")
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            args.seed = seed
            setting = "{}_{}_{}_{}_dm{}_swa{}_bc{}_lr{}_{}_seed{}".format(
                args.task_name,
                args.model_id,
                args.model,
                args.data,
        ...
        for ii in range(args.itr):
            ...
            res, ep_time = exp.test(setting, test=1)
            all_results.append(res)

            torch.cuda.empty_cache()

    macs, params = clever_format([exp.flops, exp.params], "%.2f")
    info = [macs, params, ep_time]

    result_save(all_results, info, args.model, args.data)
